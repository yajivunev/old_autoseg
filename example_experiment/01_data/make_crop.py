...
#    label_name = lookup[label_id]
    ...
    #open, read inputs
    raw = daisy.open_ds(input_zarr,'raw')
...
#            (arr_shape + padding * 2)*vs)
    ...
    raw_roi.snap_to_grid(vs)
    labels_roi.snap_to_grid(vs)
...
#            dtype=np.uint8)
    ...
    with open(os.path.join(input_zarr,f'crops/{crop_name}.zarr/crop.json'),"w") as f:
        json.dump({
            "name": crop_name,
            "raw_offset": raw_roi.offset,
            "raw_shape": raw_roi.shape,
            "labels_offset": labels_roi.offset,
            "labels_shape": labels_roi.shape
            },f,indent=4)
...
#    for x in ['raw',labels_ds,'unlabelled']:
