...
def predict(
    ...
    pred_mask = gp.ArrayKey('PRED_SDT')

    chunk_request = gp.BatchRequest()
    chunk_request.add(raw, input_size)
    chunk_request.add(pred_lsd, output_size)
    chunk_request.add(pred_sdt, output_size)
    ...
    pipeline += gp.Squeeze([raw, pred_lsd, pred_sdt])

    predict_request = gp.BatchRequest()

    predict_request.add(raw, total_input_roi.shape)
    predict_request.add(pred_lsd, total_output_roi.shape)
    predict_request.add(pred_sdt, total_output_roi.shape)

    with gp.build(pipeline):
        batch = pipeline.request_batch(predict_request)

        return batch[pred_lsd].data,batch[pred_sdt].data
...
if __name__ == "__main__":
    ...
    for z in range(sections):
        ...
        pred_lsd[z],pred_sdt[z] = predict(
                iteration,
                raw_file,
                raw_ds)

    out_lsd[total_roi] = np.swapaxes(pred_lsd,0,1)
    out_sdt[total_roi] = pred_sdt
