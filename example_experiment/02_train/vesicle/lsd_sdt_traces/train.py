...
from utils import SwapAxes, BinaryDilation, BumpBackground, UnbumpBackground, ComputeDT

from model import UnetModel, WeightedMSELoss
...
def train(
    ...
    sources = (tuple(
            gp.ZarrSource(
                ...
                datasets={
                    ...
                    unlabelled_mask: f'2d_traces_mask/{i}'},
                array_specs={
            ...
            #gp.Reject(sdt=unlabelled_mask, min_sdted=0.05) for i in sec)
    ...
    train_pipeline += BinaryDilation(unlabelled_mask,iterations=2)

    train_pipeline += gp.GrowBoundary(labels,steps=2)
    ...
    #train_pipeline += BinaryDilation(labels, iterations=1)
    ...
    train_pipeline += gp.Unsqueeze([raw, weights, gt_sdt, labels]) #c,y,x
    ...
    train_pipeline += SwapAxes([raw,weights,gt_lsd,pred_lsd,labels,gt_sdt,pred_sdt],(0,1)) #c,n,y,x
